...
/// `@Observable` models.
public protocol ArborealModel: AnyObject {
...
public protocol ArborealStore {
    associatedtype Model: ArborealModel

    /// The store's current state
    @MainActor var state: Model { get }

    /// Send action to store
    @MainActor func send(_ action: Model.Action)
}
...
actor EffectRunner<Store: ArborealStore & AnyObject> {
    /// Store to notify when effect completes.
    /// We keep a weak reference to the store, since it is expected
    ...
    /// Actions are sent to store in whatever order the tasks complete.
    public nonisolated func run(
        _ fx: Fx<Store.Model.Action>
    ) {
...
///
/// Store hold a get-only `state` which conforms to `ArborealModel`
/// and knows how to update itself and generate side-effects. All updates
/// and effects to this state happen through actions sent to `store.send`.
///
...
///
/// When using a hierarchy of observable arboreal models with Store,
...
@Observable
@MainActor
public final class Store<Model: ArborealModel>: ArborealStore {
    ...
    /// Create a store, sending an initial action
...
/// larger store.
///
/// Uses a `send` closure to forward actions up to a parent store, which
/// performs the update and manages side-effects. ViewStores can be nested many
/// levels deep, and the actions forwarded up the chain until they are
/// received by the root store.
@MainActor
public struct ViewStore<Model: ArborealModel>: ArborealStore {
    private var _send: @MainActor (Model.Action) -> Void
...
extension ArborealStore {
    ...
    @MainActor public func viewStore<ChildModel: ArborealModel>(
        get: @escaping (Model) -> ChildModel,
        tag: @escaping (ChildModel.Action) -> Model.Action
...
extension Binding {
    /// Create a `Binding` from a `ArborealStore`.
    /// - Parameters:
    ...
    @MainActor public init<Store: ArborealStore>(
        store: Store,
...
extension ArborealStore {
    /// Initialize a `Binding` from a `ArborealStore`.
    /// - Parameters:
    ///   - `get` reads the value from the state.
    ///   - `tag` tags the value, turning it into an action for `send`
    /// - Returns a binding suitable for use in a vanilla SwiftUI view.
